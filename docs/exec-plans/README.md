...
- **20260526-01-jarvis-context-engine.md**: Execution plan for replacing Jarvis's monolithic UI snapshot with a best-in-class feature-owned semantic context engine covering attention context, explicit references, typed context envelopes, prompt assembly, retrieval, budget management, and observability.
- **20260601-01-desktop-chat-stream-ownership.md**: Execution plan for moving Electron Desktop chat SSE ownership from renderer windows into the main process, adding IPC fanout, preserving the web HTTP fallback, and validating that multiple windows share one upstream chat stream.
- **20260608-01-otel-observability-runtime.md**: Execution plan for upgrading Cradle observability into a Cradle-owned OpenTelemetry runtime with traces, metrics, log/event correlation, runtime snapshots, desktop resource reporting, and guarded leak diagnostics.
- **20260619-05-renderer-context-runtime.md**: Execution plan for refactoring renderer context provider lifecycle so app composition owns provider installation, feature modules own only provider semantics, and duplicate Jarvis context registration is eliminated without exception swallowing.
