        ...
        {agents.map(a => (
          <div key={a.id}>
            ...
            <div style={{ height: 3, borderRadius: 2, background: 'var(--color-neutral-4)', overflow: 'hidden' }}>
              <motion.div style={{ height: '100%', background: a.accent, borderRadius: 2 }} animate={{ width: running ? `${Math.min(100, a.progress + 15)}%` : `${a.progress}%` }} transition={{ duration: running ? 2.8 : 0.3, ease: 'linear', repeat: running ? Infinity : 0 }} />
            </div>
          </div>
        ))}
      </div>
