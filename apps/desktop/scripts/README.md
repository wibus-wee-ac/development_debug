...
Desktop update zip artifacts are generated by Electron Builder. After `dist:mac`, generate the manifest upload directory:

    pnpm --filter @cradle/desktop generate-update-manifest -- \
      --base-url "https://updates.example.com/cradle"

Upload `apps/desktop/release/update` so the public update root contains `macos/manifest.json` and the referenced macOS zip. Set `CRADLE_DESKTOP_UPDATE_URL` to that public update root before building the app, or point it directly at the manifest JSON.
