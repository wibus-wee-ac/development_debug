...
- `Sources/CradleMacBridge/main.swift`: NDJSON protocol server, permission status checks, both-Command input monitor, frontmost window inventory, and first-pass window screenshot capture.
...
- `mac.capture.frontmostWindow`: Captures the current frontmost window to the provided output directory and shows a native top-center feedback indicator on the captured window's screen.
