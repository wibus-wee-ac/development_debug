...
- **use-global-event-listeners.ts**: App-shell side-effect hook wiring PTY pushes, keyboard shortcuts, visible-chat ownership, and global chat activity → unread reconciliation; BrowserPanel webview tab shortcuts resolve against the active Cradle tab's BrowserPanel owner id; macOS-sensitive chords like `Ctrl+\`` and `⌘⌥B` match on both `key` and `code` so layout / Option-modified keyboards stay reliable, and the chat right-aside Files shortcut shares the same first-render performance start mark as the header button.
