...
- **chat.tab.tsx**: Chat session tab（params: `sessionId`），使用通用 fallback label，在会话标题加载后替换为真实标题，并为非 CLI-TUI workspace-backed chat 注册默认激活/持久化的 bottom terminal panel、browser panel capability、right aside capability；CLI-TUI chat route 显式不注册 bottom shell，避免双 PTY 争用
