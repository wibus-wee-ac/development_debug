...
import { openTearoffSessionWindow } from './tearoff-sessions'
...
function SurfaceIcon({ surface }: { surface: AppSurface }) {
...
export const SurfaceBar = memo(({
  ...
  const [activeDragSurface, setActiveDragSurface] = useState<AppSurface | null>(null)
  ...
  const checkTearOff = useCallback((surfaceId: string | number) => {
    ...
    if (getDragDistance(dragStartPointerRef.current, releasePointer) <= TEAR_OFF_RELEASE_DISTANCE_PX) {
      return false
    }

    const surface = useSurfaceStore.getState().surfaces.find(item => item.id === surfaceId)
    const sessionId = surface ? readChatSessionId(surface) : null
    if (sessionId && releasePointer) {
      void openTearoffSessionWindow(sessionId, {
  ...
  const handleDragStart = useCallback((event: DragStartEvent) => {
    ...
    const surface = useSurfaceStore.getState().surfaces.find(item => item.id === event.active.id)
    setActiveDragSurface(surface ?? null)
  ...
  const handleDragEnd = useCallback((event: DragEndEvent) => {
    ...
    const oldIndex = currentSurfaces.findIndex(surface => surface.id === active.id)
    const newIndex = currentSurfaces.findIndex(surface => surface.id === over.id)
          ...
          {activeDragSurface && (
            <div
              className={cn(
                ...
                activeDragSurface.closable ? 'pl-3 pr-7' : 'px-3',
              )}
            >
              <span className="shrink-0">
                <SurfaceIcon surface={activeDragSurface} />
              </span>
              <span className="truncate select-none">{activeDragSurface.title}</span>
            </div>
          )}
        </DragOverlay>
