...
- **browser-panel.ts**: Right-side BrowserPanel tab state — owns lightweight browser tabs plus workspace-file/workspace-diff panel tabs, active panel tab selection, browser-use tab requests, browser tab source session metadata, per-tab enabled script ids, scoped diff scroll commands, and webview navigation metadata.
- **browser-panel.test.ts**: Regression tests for browser panel shortcuts, browser tab source metadata, per-tab script id storage, idempotent active diff tab reopening, and selector-scoped render behavior for diff scroll commands
