...
- **electron.ts**: Electron environment helpers, including main vs tear-off window metadata, typed IPC proxy for native dialogs/path launches/editor opens/browser tab script injection, server HTTP base URL resolution, and server WebSocket URL derivation
