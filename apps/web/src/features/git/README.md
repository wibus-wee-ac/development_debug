...
- **changes-panel.tsx**: Right-aside Changes panel with a Type/Tree view toggle; Type view uses the `@pierre/trees` built-in icon resolver and sprite sheet for lightweight current Git file-change rows, while Tree view renders the same changed paths through `@pierre/trees` and handles file double-clicks from the React event boundary so it does not depend on internal tree container registration timing.
- **changes-panel.test.ts**: Unit coverage for Changes panel file-section grouping order, tree event metadata resolution, and Tree view double-click diff navigation.
