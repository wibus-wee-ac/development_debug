...
- **browser-panel.tsx**: Embedded right-side panel UI with lightweight browser/workspace-file/workspace-diff tabs, selector-scoped store subscriptions, accessible tab controls, Electron `<webview>` mounting, React Activity-backed tab visibility, workspace file preview/editor/diff rendering, webview event listeners, active-session tab source markers for browser tabs created from other chat sessions, and preset/custom script controls that sync into the desktop script injection service before first navigation.
- **browser-panel.test.tsx**: Regression tests for BrowserPanel shell render boundaries around workspace diff scroll commands, cross-session browser tab source markers, stable webview refs, and initial navigation idempotence.
