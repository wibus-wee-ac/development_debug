...
- **browser-annotation-overlay.tsx**: Browser-owned screenshot annotation surface for native browser tabs; renders the captured viewport while the native `WebContentsView` is hidden, supports point/region/element anchors, element recognition outlines/search, comment/design editor modes, image attachments with preview, and grouped design adjustments for element style/layout/spacing properties.
- **browser-panel.tsx**: Mixed right-side panel UI scoped to the owning Cradle tab id; renders native browser tabs, workspace file preview/editor tabs, workspace diff tabs, browser navigation controls, address suggestions, composer screenshot attachment, browser annotation mode controls/tray, and a native viewport anchor whose bounds are synchronized to desktop `WebContentsView` only while a browser tab is active and no annotation screenshot overlay is active.
