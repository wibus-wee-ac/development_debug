...
export function RelationManager({ issueId, workspaceId }: RelationManagerProps) {
  const { data: relations = [] } = useRelations(issueId)
  const { data: workspaceIssues = [], isLoading: isLoadingWorkspaceIssues } = useIssues({ workspaceId })
  ...
  const typedIssue = useMemo(() => {
    ...
    return candidateIssues.find((issue) => {
      const readableId = formatIssueId(issue, workspaces).toLowerCase()
      return issue.id.toLowerCase() === needle
        || readableId === needle
  ...
  const selectedIssue = targetIssueId
    ...
    : typedIssue
  const resolvedTargetId = targetIssueId ?? typedIssue?.id ?? (trimmedQuery.length > 0 ? trimmedQuery : null)
  const resolvedTargetLabel = selectedIssue ? `${formatIssueId(selectedIssue, workspaces)} ${selectedIssue.title}` : resolvedTargetId
  ...
  const submitRelation = (action: RelationAction) => {
    ...
    const targetIssueIdForRelation = action.direction === 'current-source' ? resolvedTargetId : issueId
  ...
  return (
    <div className="space-y-3">
      {relationSections.map((section) => {
        ...
        return (
          <section key={section.key} className="space-y-1.5" aria-labelledby={`issue-${section.key}-heading`}>
            <div className="flex items-center justify-between">
              <h3 id={`issue-${section.key}-heading`} className="text-[12px] font-medium text-muted-foreground">
                {section.label}
              </h3>
              <Popover
                  ...
                  <div className="space-y-2">
                    ...
                    >
                      <ComboboxInput
                        ...
                        showTrigger
                        size="sm"
                        startAddon={<SearchIcon className="size-3 text-muted-foreground" aria-hidden="true" />}
                        className="w-full"
                      />
                      <ComboboxContent align="start" sideOffset={6} className="w-72 p-1.5">
                        <ComboboxList className="max-h-56 p-0.5">
                          {(isLoadingWorkspaceIssues || searchIssues.isFetching) && <Skeleton className="h-10 w-full" />}
                          {!isLoadingWorkspaceIssues && !searchIssues.isFetching && candidateIssues.length === 0 && (
                            <div className="px-2 py-5 text-center text-xs text-muted-foreground">
                              Type an issue ID to link directly
                            </div>
                          )}
                          {!isLoadingWorkspaceIssues && candidateIssues.map(issue => (
                    ...
                    <div className="rounded-lg bg-fill/40 px-2 py-1.5 text-[11px] text-muted-foreground">
                      {resolvedTargetLabel ? `Target: ${resolvedTargetLabel}` : 'Choose a target issue first'}
                    </div>
...
function RelationRow({
  ...
  const targetId = relation.sourceIssueId === issueId ? relation.targetIssueId : relation.sourceIssueId
...
function IssueRelationComboboxItem({ issue, readableId }: { issue: KanbanIssue, readableId: string }) {
  return (
    <ComboboxItem
      value={issue.id}
      className="min-h-11 items-start gap-2 rounded-lg p-2 pr-7"
    >
      <span className="mt-0.5 shrink-0 font-mono text-[11px] text-muted-foreground tabular-nums">{readableId}</span>
