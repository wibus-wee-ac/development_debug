...
- **use-agent-models.ts**: `useAgentModels`, `useProviderTargetModels`, and `useAgentModelMap` hooks — read cached visible models for one manual profile, one provider target, or a profile-keyed composer model map, share stable query keys across chat, composer, and settings surfaces, avoid automatic provider inventory refreshes, and expose per-profile query success for Settings Jarvis readiness
