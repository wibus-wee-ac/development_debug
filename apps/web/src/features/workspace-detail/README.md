...
- **capsule-composer.tsx**: Workspace overview composer，复用 shared persisted new-chat preference state 与 chat-owned attachment primitives，避免组件自己直接读写 localStorage 或复制附件状态机；发送图标按钮暴露稳定英文 accessible name，并为 workspace-originated task journey 提供最小稳定 E2E anchors
