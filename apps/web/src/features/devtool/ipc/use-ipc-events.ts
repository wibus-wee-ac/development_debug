...
export function useIpcTraces(): IpcTrace[] {
  ...
  return useMemo(() => {
    ...
    for (const event of events) {
      ...
      if (event.callerStack.length > 0) {
        trace.callerStack = event.callerStack
      }
      if (event.flowId !== undefined) {
        trace.flowId = event.flowId
      }
    }
...
function deriveStatus(trace: IpcTrace): IpcObservedStatus {
  ...
  if (rendererFinish) {
    return rendererFinish.status
  }
  const mainFinish = trace.phases['main:finish']
  if (mainFinish) {
    return mainFinish.status
  }
  return 'pending'
}
...
export function useIpcFilteredTraces(): IpcTrace[] {
  ...
  return useMemo(() => {
    ...
    return traces.filter((trace) => {
      if (!filters.statuses[trace.status]) {
        return false
      }
