...
- **resources-popover.tsx**: ResourcesPopover trigger and popover content; samples renderer memory, server health, terminal resource snapshots, and Chronicle resource state; exposes accessible trigger/refresh controls, surfaces partial endpoint failures instead of silently showing zero values, and records the resources popover first-render performance gate once all resource endpoints are ready after user open.
