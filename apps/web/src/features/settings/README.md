...
- **about-settings.tsx**: 关于设置页，使用紧凑 Settings row 布局说明 Cradle-owned Application Support 数据目录，并明确列出用户确认或显式操作后可能写入 Application Support 外的 workspace、skills 与 CLI command 路径。
...
- **shortcuts-settings.tsx**: Shortcuts 设置页，读取 Desktop preferences 并控制 desktop-owned AppShot `bothCommand` 触发是否启用；保存后通过 Electron IPC 立即同步 mac bridge input configuration。
...
- **desktop-update-settings.tsx**: Desktop 设置页，通过 Electron preload / IPC 管理 `electron-updater` 更新状态、检查、下载与应用，并提供 packaged macOS `cradle` PATH command 的显式 install/repair/remove 入口；Settings Desktop 首屏在 update status 与 CLI status 初始化完成后记录 performance gate。
