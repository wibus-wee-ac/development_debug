...
Composer-adjacent slot renderers for provider-owned runtime UI state. The parent `composer-slot-states.tsx` owns slot selection by declared `composerState` surface; files in this directory own the concrete rail UI for each slot kind.
...
- **usage-slot-state.tsx**: ChatGPT account usage rail backed by provider-projected rate-limit windows, percentages, reset timestamps, and credits.
