...
- **appshot-attachment.tsx**: Cradle-owned AppShot file-part metadata reader, creator, and composer/thread visual card; keeps the model input as a normal image file while renderer UI reads `providerMetadata.cradle.appshot` for Codex-like AppShot presentation semantics.
