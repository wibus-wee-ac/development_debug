...
- **appshot-attachment.tsx**: Cradle-owned AppShot composer/thread visual card; composer rendering keeps the native transition snapshot aligned with the final card, falls back to the final capture image when needed, and shows the same app icon/title identity as the native handoff, while thread rendering uses the final capture image with Codex-like centered image, mask, shadow, sibling icon overlay, and caption treatment.
...
- **composer-action-context.ts**: Composer-owned UI action geometry reader; measures title-aware AppShot pending slots and emits renderer-owned `viewportPixels` animation targets for native capture.
