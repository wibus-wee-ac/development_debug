...
- **agent-list.tsx**: Agent 列表，显示所有 Agent 卡片；点击行导航到 agent-detail；支持 Settings overlay 的一次性 Agent focus target，用于从 Smart Mention 等外部入口直接定位对应 Agent；列表行现在展示绑定的 provider 名称，避免多个同类 provider 时无法分辨归属；draft row 使用即时布局挂载，避免列表高度动画；Settings Agents 首屏在 agents 与 profiles 两条 server-backed query 成功后记录 performance gate
