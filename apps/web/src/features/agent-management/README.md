...
- **agent-detail.tsx**: Agent 详情页，提供内联编辑 identity、provider/model/thinking 统一选择器、system prompt、CLI TUI env 输入反馈与 agent-private Skills 管理，并确保 provider 切换时默认模型与 thinking 能力会同步到 state
- **agent-detail.test.ts**: Agent detail 的纯函数契约测试，覆盖 CLI TUI env 解析反馈与创建按钮禁用原因
...
- **profile-detail-panel.tsx**: Provider 详情面板，继续以 RHF 作为表单 owner，并把模型加载 / registry 映射 / 健康检查 / 自动保存 / 删除确认等瞬时 UI 状态收口到局部 reducer，避免细碎 `useState` 级联
