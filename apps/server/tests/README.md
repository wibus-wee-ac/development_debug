...
- **chat-runtime.test.ts**: chat run execution, timeline hydration, usage writes, and abort flow.
...
- **sdk-providers.test.ts**: unified Claude Agent / Codex metadata probing, model listing, and `/chat` execution flows.
...
- **search.test.ts**: thread search over titles, user content, and assistant timeline text.
