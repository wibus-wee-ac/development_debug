...
describe('relay servers', () => {
  ...
  it('pairs a relay host through a configured relay server id', async () => {
    ...
    try {
      ...
      const tokenRes = await app.handle(new Request('http://localhost/remote-runtime-hosts/remote-host-relay-server-pairing/relay/pairing-token', {
        method: 'POST',
        headers: { 'content-type': 'application/json' },
      ...
      const errRes = await noDefaultApp.handle(new Request('http://localhost/remote-runtime-hosts/remote-host-no-relay/relay/pairing-token', {
        method: 'POST',
        headers: { 'content-type': 'application/json' },
