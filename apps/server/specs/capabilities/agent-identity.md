...
- 系统需要维护可供会话与运行时绑定的 Agent 实体（名称、头像、agent profile 关联、模型与思考强度）。
...
- 旧 IPC `list()` 返回全量，调用方通常再按 `enabled` 或 `agentProfileId` 做前端过滤。
- `agentProfileId` 关联 `agent_profiles.id`，非法 profile 由 DB 约束拒绝。
...
- `agentProfileId?: string` → 仅返回绑定到指定 agent profile 的 Agent

## Dependencies

- `@cradle/db`：`agents` / `agent_profiles`。
- `DbAccessor`（server DB 访问）。
- `AppError`（输入校验和错误归一）。

## Target API

- `GET /agents?enabled=&agentProfileId=` → `Agent[]`
...
- `agentProfileId` 非法或不存在：`AppError`（`agent_profile_not_found`, HTTP 400）
...
- 保持与旧 IPC 关键语义一致：默认值、头像 URL 生成规则、`update` 局部更新行为。
...
- 过滤：`enabled` / `agentProfileId` query 生效。
- 输入校验：缺失必填字段返回 400 + `invalid_agent_input`。
- 非法 agentProfileId 返回 400 + `agent_profile_not_found`。
