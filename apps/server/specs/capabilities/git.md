...
- 用户需要基于真实 workspace 查看当前 Git 分支、浏览分支列表、切换/创建分支，并在 Git 面板查看真实提交图。
- server-first 版本必须避免 `workspacePath` 泄漏到客户端；Git API 统一以 `workspaceId` 为入口，由服务端解析仓库路径。
- 第一阶段交付只覆盖读状态 + branch control + commit graph，不把 Git 做成完整客户端。
...
- `file-statuses`、commit/push/pull/diff 等更重语义暂时后置。

## Test Plan

- 真实临时仓库可返回当前分支、分支列表、提交图（包含 `--all` 历史）。
- 创建新分支会立即切换到新分支。
- checkout 可切换现有分支。
