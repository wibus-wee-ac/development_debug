...
- 系统需要提供跨 `builtin` / `legacy` / `global` / `workspace` / `agent` scope 的 skills 清单与文档读取。
- HTTP client 需要支持技能 CRUD、导入导出，以及从本地或 git source 发现并导入 skill package。
- server 必须保持 namespace ownership：只读消费 `builtin` / `legacy`，只写入 Cradle 自己拥有的 `global` / `workspace` / `agent` 作用域。
...
- `builtin` 与 `legacy` scope 为只读；`global` / `workspace` / `agent` 为可写。
