...
- 系统需要提供跨 `builtin` / 标准 `.agents` / Cradle-only / `workspace` / `agent` scope 的 skills 清单与文档读取。
- HTTP client 需要支持技能 CRUD、导入导出，以及从本地或 git source 发现并导入 skill package。
- server 必须保持 namespace ownership：只读消费 `builtin` 与标准 `.agents` skills，只写入 Cradle 自己拥有的 Cradle-only / `workspace` / `agent` 作用域。
...
- `builtin` 与标准 `.agents` scope 为只读；Cradle-only / `workspace` / `agent` 为可写。
...
- scope precedence 与 `shadowedBy` 正确返回。
