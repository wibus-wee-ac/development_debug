...
- 系统需要为每个 workspace 保存一份全局 workflow rule，以及若干 agent-profile 级别的覆盖规则。
...
- 规则存储为 Markdown 文本，global 固定为 `rules.md`，agent-specific 固定为 `agents/<agentProfileId>.md`。
- 旧实现对 workspaceId / agentProfileId 做路径穿越保护，非法 ID 直接抛错。
- 缺失文件时读取返回 `null`，删除为幂等。

## Inputs / Outputs

- `get(workspaceId, agentProfileId?)` → `{ global: string | null, profileSpecific: string | null }`
- `save(workspaceId, agentProfileId | null, content)` → `{ ok: true }`
- `delete(workspaceId, agentProfileId | null)` → `{ ok: true }`
- `list(workspaceId)` → `Array<{ type: 'global' | 'agent', agentProfileId: string | null, content: string }>`
...
- `GET /workflow-rules/:workspaceId?agentProfileId=` → `WorkflowRules`
- `GET /workflow-rules/:workspaceId/list` → `WorkflowRuleEntry[]`
- `PUT /workflow-rules/:workspaceId` `{ agentProfileId?: string | null, content: string }` → `{ ok: true }`
- `DELETE /workflow-rules/:workspaceId?agentProfileId=` → `{ ok: true }`

错误约定：

- 非法 workspaceId / agentProfileId → `invalid_workflow_rule_id`, HTTP 400
