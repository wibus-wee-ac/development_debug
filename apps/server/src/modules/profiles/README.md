...
- `profiles.service.ts` — coordinates saved profile semantics, preserves profile config fields, stores profile-owned models.dev mappings, allows Cradle-owned model settings on external-source mirrored profiles while blocking source-owned edits, and handles session cleanup.
