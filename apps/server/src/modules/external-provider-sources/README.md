...
- `service.ts`: Snapshot validation, source persistence, direct onboarding source refresh, runtime target projection, secret upsert, missing-record handling, runtime preference preservation, disabled-target agent shutdown, and view serialization.
...
When an external runtime target is disabled explicitly or marked disabled because its source record disappeared, this module also disables agents bound to that provider target so launchability stays consistent with provider availability.
