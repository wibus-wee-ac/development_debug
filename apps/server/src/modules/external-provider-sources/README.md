...
- `service.ts`: Snapshot validation, source persistence, profile projection, secret upsert, missing-record handling, and view serialization. Projection keeps source-owned connection fields separate from Cradle-owned model visibility and models.dev mappings.
...
Plugins do not render Provider UI and do not write Cradle profile or credential tables directly. They register external provider sources through `@cradle/plugin-sdk/server`; this module reads the registered sources and writes Cradle-owned projection state. External sources may supply provider connection data and credentials, while provider model lists, custom models, visibility, and cost metadata remain owned by Cradle provider/profile modules.
