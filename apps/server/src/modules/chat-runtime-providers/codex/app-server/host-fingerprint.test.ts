...
describe('createCodexAppServerHostFingerprint', () => {
  it('excludes thread-level config from fingerprint', () => {
    ...
    const fp2 = createCodexAppServerHostFingerprint({
      options: {
        ...
        env: {
          CRADLE_CHAT_SESSION_ID: 'session-2', // different
          CRADLE_WORKSPACE_ID: 'workspace-2', // different
        },
      },
      chatgptAuth: null,
    })

    // Same process-level config (apiKey, no model_provider) should produce same fingerprint
    expect(fp1).toBe(fp2)
  })
