...
export async function readClaudeAgentAuthDiagnostics(
  ...
  if (config.authMode === 'apiKey') {
    return readClaudeApiKeyAuthDiagnostics({ providerTargetId: target.id, credentialRef: target.credentialRef }, config, deps)
  }

  return await readClaudeAiAuthDiagnostics({ providerTargetId: target.id }, deps)
}

function readClaudeApiKeyAuthDiagnostics(
  ...
  try {
    ...
    return {
      ...
      version: null,
      message: `Claude API key credential could not be read: ${errorMessage(error instanceof Error ? error : new Error(String(error)))}.`,
      account: null,
    }
  }
  ...
  return {
    ...
    authLabel: 'Claude API Key',
    version: null,
    message: hasApiKey ? null : 'Claude API key authentication is selected, but no API key is configured.',
    account: null,
  }
}
