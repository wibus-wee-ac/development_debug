...
import * as RemoteRuntimeHosts from '../../remote-runtime-hosts/service'
...
export class RemoteMockProvider implements ChatRuntime {
  ...
  async resumeChatSession(input: ResumeChatSessionInput): Promise<RuntimeSession> {
    ...
    if (snapshot.remote) {
      RemoteRuntimeHosts.upsertRemoteRuntimeSessionLink({
  ...
  async* streamTurn(input: StreamTurnInput): AsyncGenerator<UIMessageChunk, void, void> {
    ...
    try {
      await RemoteRuntimeHosts.connectRemoteRuntimeHost(link.remoteHostId)
      const stream = RemoteRuntimeHosts.openRemoteRuntimeHostStream(link.remoteHostId, 'agent/turn', {
  ...
  async steerTurn(input: SteerTurnInput): Promise<void> {
    const link = RemoteRuntimeHosts.readRemoteRuntimeSessionLink(input.runtimeSession.chatSessionId)
    if (!link) {
    ...
    try {
      await RemoteRuntimeHosts.callRemoteRuntimeHost(link.remoteHostId, 'agent/steer', {
        remoteAgentId: link.remoteAgentId,
        message: input.message,
      })
    }
  ...
  async cancelTurn(input: CancelTurnInput): Promise<void> {
    ...
    try {
      await RemoteRuntimeHosts.callRemoteRuntimeHost(link.remoteHostId, 'agent/cancel', {
        remoteAgentId: link.remoteAgentId,
  ...
  async healthCheck() {
    return {
      ...
      message: 'Remote mock health is host-specific; use /remote-runtime-hosts/:hostId/health.',
      lastCheckedAt: Math.floor(Date.now() / 1000),
    }
  }

  private async resolveRemoteLink(input: StreamTurnInput) {
    const existing = RemoteRuntimeHosts.readRemoteRuntimeSessionLink(input.runtimeSession.chatSessionId)
    if (existing) {
      return existing
    }
    ...
    const link = RemoteRuntimeHosts.readRemoteRuntimeSessionLink(session.chatSessionId)
    if (!link) {
      throw new ProviderRuntimeError(
  ...
  private async startRemoteSession(input: {
    ...
    try {
      const connection = await RemoteRuntimeHosts.connectRemoteRuntimeHost(config.remoteHostId)
      const result = await RemoteRuntimeHosts.startRemoteAgent(config.remoteHostId, {
