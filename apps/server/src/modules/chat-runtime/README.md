...
Passive observers hydrate through `GET /chat/sessions/:sessionId/messages`, then join the currently active run through `GET /chat/sessions/:sessionId/stream`; the session stream starts at subscription time and does not replay chunks already covered by the snapshot response.
...
- `providers/`: concrete runtime providers grouped by backend owner.
  ...
  - `providers/claude-agent/provider.ts`: Claude Agent SDK runtime bound to the unified `/chat` API; injects plugin-registered MCP servers and `config.claudeAgent.modelAliases` environment overrides into Claude Agent SDK query options, projects text plus image file parts into Claude Agent SDK user message content blocks, and rejects unsupported non-image file parts at the provider boundary.
