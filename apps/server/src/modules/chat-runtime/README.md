...
`POST /chat/sessions/:sessionId/cancel` owns the canonical aborted transition: it marks active runs/messages terminal before asking the provider to stop, ignores late provider chunks after terminal state, and repairs persisted `streaming` rows when no in-memory active run exists. `GET /chat/sessions/:sessionId/messages` also repairs orphaned persisted `streaming` rows when there is no active or pending in-memory run, so reload recovery does not expose stale in-progress state after a process exit.
...
Chat Session queue and steer are owned here: `/chat/sessions/:sessionId/queue` stores follow-up turns in `chat_session_queue_items`, applies true live steering only when the active provider exposes the optional `steerTurn` hook, persists visible continuation messages with `metadata.cradle.continuation`, otherwise drains `steer` items before normal `queue` items after the active run completes, and does not write into Issue Agent namespaces.

## Files

- `index.ts`: Elysia route surface for `POST /chat/sessions/:sessionId/response`, `GET /chat/sessions/:sessionId/stream`, `GET /chat/sessions/:sessionId/queue`, `POST /chat/sessions/:sessionId/queue`, `POST /chat/sessions/:sessionId/queue/reorder`, `DELETE /chat/sessions/:sessionId/queue/:queueItemId`, `GET /chat/sessions/:sessionId/capabilities`, `GET /chat/sessions/:sessionId/messages`, `GET /chat/runs/:runId/trace`, `GET /chat/sessions/:sessionId/traces`, and `POST /chat/sessions/:sessionId/cancel`.
- `model.ts`: HTTP params/body schemas for the chat runtime surface, including optional text plus AI SDK file parts, optional AI SDK `messages` history for approval continuation, optional `providerTargetId` overrides for response/queue requests, and decoded chat stream trace responses.
- `service.ts`: active-run orchestration, runtime-native capabilities discovery, duplicate-run reservation, provider target override resolution with runtime/provider compatibility validation, provider target + agent + session runtime config merging, Chronicle long-term memory context injection, text/file draft `UIMessage` persistence, AI SDK assistant-message continuation for native approval responses, durable queue item list/enqueue/cancel/reorder/drain, continuation metadata persistence for visible steer/queue messages, direct AI SDK `UIMessageChunk` SSE publication, terminal-only snapshot persistence through `readUIMessageStream`, trusted snapshot hydration with orphaned streaming repair, session-level passive stream subscription with active-run replay, cancellation terminal-state ownership, persisted streaming cleanup, completion subscription, trace retrieval, and event broadcasting.
...
- `providers/`: concrete runtime providers grouped by backend owner.
  ...
  - `providers/claude-agent/provider.ts`: Claude Agent SDK runtime bound to the unified `/chat` API; injects plugin-registered MCP servers, strips host model env fallbacks, pins the resolved model from the runtime session snapshot, and applies trimmed `config.claudeAgent.modelAliases` environment overrides into Claude Agent SDK query options; projects text plus image file parts into Claude Agent SDK user message content blocks and rejects unsupported non-image file parts at the provider boundary.
  - `providers/claude-agent/mapper.ts`: Claude Agent SDK message → AI SDK `UIMessageChunk` mapper; subagent child messages are projected with isolated mapper state into accumulated `UIMessage` snapshots and attached to the parent tool as preliminary `tool-output-available` chunks instead of client-routed subagent delta events.
  ...
  - `providers/codex/app-server-mapper.ts`: Codex app-server notification → AI SDK `UIMessageChunk` mapper.
