...
- `providers/`: concrete runtime providers grouped by backend owner.
  ...
  - `providers/claude-agent/mapper.ts`: Claude Agent SDK message → AI SDK `UIMessageChunk` mapper; subagent child messages are projected into accumulated `UIMessage` snapshots and attached to the parent tool as preliminary `tool-output-available` chunks instead of client-routed subagent delta events.
