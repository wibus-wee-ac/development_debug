...
- `index.ts`: HTTP routes for reading and updating provider-target model settings, model visibility, custom model IDs, and provider-target scoped Codex account diagnostics.
- `model.ts`: Elysia request and response schemas for provider-target preference and diagnostics routes.
