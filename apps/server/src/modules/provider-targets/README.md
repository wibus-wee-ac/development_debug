...
Codex account diagnostics are exposed as provider-target scoped Settings APIs under `/provider-targets/:providerTargetId/codex/*`. The routes read Codex-native account identity, auth-required state, usage, and rate-limit state only after an explicit user action in Settings; auth tokens are never projected into the diagnostics response. Unsupported targets return an unsupported diagnostics response without starting Codex app-server, while reset-credit consumption is a separate POST action with an idempotency key.
