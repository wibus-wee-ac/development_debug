...
- **workspace-sidebar.tsx**: Sidebar component whose workspace name opens the detail tab for that workspace while the folder icon controls collapse state and the workspace / session menus handle重命名、pin、Markdown copy、删除等动作；会话行与关键导航入口（如 `nav-new-chat`、`nav-kanban`、`nav-usage`）暴露稳定的 `data-testid` 锚点供 E2E 回归使用
