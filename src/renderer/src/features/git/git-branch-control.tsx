...
export function GitBranchControl({ workspacePath }: GitBranchControlProps) {
  const { data: status, isError } = useGitStatus(workspacePath)
  const createDialogRef = useRef<CreateBranchDialogHandle>(null)

  if (!workspacePath || isError || !status) {
    return null
  }
