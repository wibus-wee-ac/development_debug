// Input: useGitBranches, ipc proxy, useQueryClient, coss Popover primitives, lucide icons
...
export function BranchPicker({
  ...
  const handleCheckout = useCallback(async (branch: string) => {
    setOpen(false)
    try {
      await ipc!.git.checkout(workspacePath, branch)
      invalidateAll()
    }
    catch (err) {
  ...
  const handleCreate = useCallback(async () => {
    ...
    setCreateError(null)
    try {
      await ipc!.git.createBranch(workspacePath, name)
      invalidateAll()
      setOpen(false)
      setCreating(false)
    }
  ...
  }, [newName, workspacePath, invalidateAll])
