...
on the `chat:timeline-event` IPC channel, consumed through the preload `chatPush` wrapper
and the unified `use-chat-events` hook. The transport no longer knows backend-native
event shapes; it simply consumes chat projection chunks, while reload/reconnect paths
fall back to persisted snapshot observation so mid-flight refreshes do not corrupt the
...
- **ipc-chat-transport.ts**: `ChatTransport` implementation bridging AI SDK's useChat to our IPC — `sendMessages` → `ipc.chat.send` + subscribe to `chatPush.onTimelineEvent`; guards stream teardown races so late IPC events from an old page do not throw after navigation; consumes projected `UIMessageChunk`s directly
- **use-chat-events.ts**: Unified chat event bridge — single preload subscription, multi-consumer dispatch via `useChatTimelineEvent`, `useGlobalChatTimelineEvent`, and `useChatSessionTitle` hooks
