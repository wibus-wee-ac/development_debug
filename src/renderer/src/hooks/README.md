...
- **use-global-event-listeners.ts**: App-shell side-effect hook wiring PTY pushes, chat timeline completion notifications, and keyboard shortcuts; macOS-sensitive chords like `Ctrl+\`` and `⌘⌥B` match on both `key` and `code` so layout / Option-modified keyboards stay reliable
