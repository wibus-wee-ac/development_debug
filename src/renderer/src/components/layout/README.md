...
- **app-layout.tsx**: Main three-column layout shell — sidebar, center column, aside, and bottom panel. Accepts `aside` and `panel` as composition props so each page can inject its own content. Also renders `DevBottomBar` at the bottom in dev mode.
