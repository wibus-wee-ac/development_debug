...
export function createKanbanQueryApplicationService(
  ...
  const getAgentSessions: KanbanQueryApplicationService['getAgentSessions'] = (issueId) => {
    return sortDescByCreatedAt(store.listAgentSessionsByIssue(issueId))
  }

  const getAgentActivities: KanbanQueryApplicationService['getAgentActivities'] = (agentSessionId) => {
