...
- **openai-compatible-provider.ts**: OpenAICompatibleProvider validates Base URL/API key profile config, streams chat completions, maps deltas into typed timeline facts, and normalizes user-triggered aborts so cancelled turns do not finalize as completed.
