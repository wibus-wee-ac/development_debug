...
- **chat.ts**: IPC service forwarding to ChatEngine — `createAndSend`, `send`, `abort`, `getMessages`, `ensureLive` (returns live ACP attachment plus continuity mode)
...
- **session.ts**: IPC service for persisted chat sessions, recoverable ACP session handles, and per-session config snapshots; message writes are owned by ChatEngine (this service only reads)
