...
These modules manage process spawning, protocol connections, chat orchestration, and Electron services.
...
- **chat-engine.ts**: ChatEngine singleton — sole orchestrator for chat sessions; owns transactional user+assistant writes, OpenAI-style `chat:response-event` IPC broadcast, debounced DB flush, abort/failure semantics, crash-recovery on init, ACP session restore before reset, and ACP error detail expansion for chat failures
