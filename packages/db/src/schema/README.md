...
- **chat.ts**: Product session、message、usage log、Chat Session continuation queue 相关表；`sessions.archived_at` 是 Session-owned 软归档状态，默认列表隐藏但不删除 messages/usage/runtime history；`messages.message_json` 是 chat hydration 真相源，`messages.content` 是派生纯文本 cache，`chat_session_queue_items` 由 Chat Runtime 拥有，用于持久化 `queue` / `steer` follow-up、chat-owned context parts 与 Claude Agent `bypassPermissions` / `plan` mode
