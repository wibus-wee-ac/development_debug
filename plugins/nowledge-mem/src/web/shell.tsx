...
export function NowledgeShell({ isActive, ctx }: NowledgeShellProps) {
  ...
  const [animatedTab, setAnimatedTab] = useState<NowledgeTab | null>(null)
  ...
  const activateTab = (tabId: NowledgeTab) => {
    ...
    setAnimatedTab(tabId)
    setNowledgeUiState({ activeTab: tabId })
  }

  const animatePanel = animatedTab === activeTab
              ...
              {TABS.map(({ id, label, icon: Icon }) => {
                ...
                return (
                  <button
                    ...
                    aria-label={label}
                    title={isActiveTab ? undefined : label}
