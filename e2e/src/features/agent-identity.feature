...
功能: Agent Identity 管理
  ...
  场景: 完整创建 Agent 并持久化
    ...
    而且 数据库中应持久化名称为"Planner Agent"、Provider 为"Primary Provider"、Model 为"gpt-4o-mini"、Thinking Effort 为"high"的 Agent

  @CRADLE-AGENT-IDENTITY-006
  ...
  场景: 编辑 Agent 名称与 Thinking Effort
    ...
    那么 数据库中应持久化名称为"Draft Agent Updated"、Provider 为"Primary Provider"、Model 为"gpt-4o-mini"、Thinking Effort 为"low"的 Agent
    当 我返回 Agent 列表
    那么 Agent 列表中应显示名称为"Draft Agent Updated"、Provider 为"Primary Provider"、Model 为"gpt-4o-mini"的条目

  @CRADLE-AGENT-IDENTITY-008
  场景: 删除已有 Agent
    假如 我已准备名为"Primary Provider"模型为"gpt-4o-mini"的 Agent Provider
    ...
    而且 数据库中不应存在名称为"Disposable Agent"的 Agent

  @CRADLE-AGENT-IDENTITY-009
  场景: 切换 Provider 后 Agent Model 应联动更新
    ...
    当 我点击创建 Agent 保存按钮
    那么 数据库中应持久化名称为"Provider Bound Agent"、Provider 为"Beta Provider"、Model 为"beta-model"、Thinking Effort 为"auto"的 Agent
